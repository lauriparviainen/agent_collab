...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "earlier event or full fidelity is needed (each line carries its event_id). timeout_ms "
            "bounds one server-side block (default 45000, max 600000; above ~45000 clients kill "
            "the call near 60 s); on expiry it returns a heartbeat (settled=false, no answers) — "
            "re-poll immediately, the 20s pacing rule does not apply. timeout_ms=0 never blocks: "
            "an instant peek of the current state, the cheap sweep across several delegated "
            "sessions. On an already-terminal session it returns immediately, so this is also the "
            "cheap harvest after a wait_events watch loop: never rebuild a result by re-reading "
            "message events. You are unresponsive for the whole block, so as a collector use it "
            "only when you want nothing but the outcome; otherwise watch with wait_events, then "
            "call this."
        ),
        "inputSchema": {
            "type": "object",
