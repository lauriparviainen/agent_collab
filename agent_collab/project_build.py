"""Repository build checks and generated daemon REST API documentation."""
...
_FIELD_SCHEMAS: Dict[Tuple[type, str], Dict[str, Any]] = {
    ...
    (SessionResultModel, "pending_approvals_omitted"): {"minimum": 0},
    (SessionStateModel, "pending_approvals_omitted"): {"minimum": 0},
    (SessionStopModel, "approvals_denied"): {"minimum": 0},
...
def render_http_api(schema: Optional[Mapping[str, Any]] = None) -> str:
    ...
    lines.extend(
        [
            ...
            "`./agent_collab_dev.sh build`; verify them without writes using `./agent_collab_dev.sh build --check`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def run_build(
    ...
    *,
    ...
    if drift:
        ...
        raise BuildError(
            f"generated build artifacts are stale: {names}; run ./agent_collab_dev.sh build"
        )
...
def _operation(route: Any) -> Dict[str, Any]:
    ...
    if protected:
        responses["401"] = error
    if route.handler in {"resolve_approval", "resume_session", "interrupt_session"}:
        responses["409"] = error
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate repository config and generate API artifacts."
    )
    parser.add_argument(
    ...
    from .cli_output import error, ok

    try:
        agents, workflows = run_build(args.workdir, check=args.check)
    except (OSError, ValueError, BuildError) as exc:
        error(str(exc))
        return 1
    action = "Verified" if args.check else "Generated"
    ok(
        f"Config valid: {args.workdir.expanduser().resolve()} ({agents} agents, {workflows} workflows)"
    )
    ok(f"{action}: {OPENAPI_PATH}")
    ok(f"{action}: {HTTP_API_PATH}")
    return 0
