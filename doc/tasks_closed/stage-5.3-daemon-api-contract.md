...
Status: **Complete — closed 2026-07-10.** Workstreams A and B are complete; the
OpenAPI artifacts are generated by `./agent_collab_dev.sh build`, and token rotation
on every daemon start is the intended final behavior. Grounding verified against
the implementation and full hermetic suite. See
...
- **Stdlib-only contract tooling.** The repository now requires Python >= 3.10
  and installs first-class provider SDK dependencies, but the HTTP DTO and
  OpenAPI setup path adds no schema/codegen dependency. No `pydantic` or OpenAPI
  codegen library: the contract and generator use `dataclasses`, `typing`, and
  `json`.
...
**Implemented as setup output.** `./agent_collab_dev.sh build` validates the
effective workdir config and generates `doc/daemon_api_doc/http-api.md` plus
`doc/daemon_api_doc/openapi.json` from the DTOs and `ROUTES` registry.
`./agent_collab_dev.sh build --check` verifies those artifacts without writing, so
CI can detect drift. The DTOs remain the runtime source of truth; the generated
files are documentation output.
...
- Generate `doc/daemon_api_doc/http-api.md` and
  `doc/daemon_api_doc/openapi.json` for the REST-shaped routes during
  `./agent_collab_dev.sh build`; `build --check` provides the regen-and-diff gate.
...
### Workstream A completion (2026-07-10)
- **Done: table-driven `_dispatch` off `ROUTES`** — closes reverse route-completeness
  (server dispatch ⊆ `ROUTES`) fully. Handler names now live on the registry,
  and contract tests require every entry to resolve to a server handler.
- **Done: malformed numeric/null request fields → `400`, not `500`.** Shared DTO
  numeric helpers now translate both `TypeError` and `ValueError` into a
  validation `ValueError`; REST maps that to `400`, and MCP uses the same query
  DTOs for equivalent validation.
- **Done: update [daemon-architecture.md](../daemon-architecture.md) "Suggested
  endpoints"** — the stale endpoint list was replaced with a pointer to
  `api_schema.ROUTES` and the shared DTOs as the source of truth.
- **Done:** generated `doc/daemon_api_doc/http-api.md` and `openapi.json` via
  `./agent_collab_dev.sh build`, with a non-writing `build --check` drift gate.
- **Done: tool-output elision on read (`tool_output` parameter)** — requested
...
- **Rotate:** **Decision — per-daemon-lifetime only.** Each start mints a new
  token, atomically superseding the old file. This per-start rotation is the
  intended final behavior; timer-based rotation is not planned. The client's
  `401` re-read-and-retry-once path (below) covers the restart-while-client-lives
  case.
...
### Progress: Workstream B landed

**Done (2026-07-10):** the serving process mints an atomic owner-only token for
each lifetime; every route except `GET /health` requires Bearer auth; the typed
client attaches/reloads the local token; supervisor readiness proves protected
`GET /sessions`; daemon state files and directory permissions are tightened;
and direct HTTP MCP migration guidance now recommends the token-aware stdio
adapter for local clients. The final hermetic suite passes 494 tests and an
isolated real daemon auth/permissions round-trip passes.

Reviewed through Agent Collab with xAI `grok-build`: first-pass
`daemon-b259995c1f8e43e4` returned **SIGN-OFF** with minor observations; all
actionable observations were tightened; second-pass
`daemon-8f4580f8e94b46b6` returned **SIGN-OFF** with no blockers. The later
`setup`/OpenAPI extension was validated by xAI `grok-build` in
`daemon-ae6906abffbb428c`, which also returned **SIGN-OFF**.
...
Reviewed by solo-codex through agent-collab on 2026-07-09
(`daemon-3de1d98eb7e24974`). Folded in: demote the hand-rolled
...
resolved into [Resolved Decisions](#resolved-decisions): A initially prioritized
DTOs + tests (the setup generator landed afterward); wrap **all** REST routes; version via health field +
...
- **A scope:** typed DTOs + contract tests are the runtime source of truth.
  `./agent_collab_dev.sh build` generates `doc/daemon_api_doc/http-api.md` and
  `openapi.json`; `build --check` verifies drift without writing.
...
- **Versioning:** `GET /health` version field **+** `X-Agent-Collab-API: 2`
...
- **Rotation:** **per-daemon-lifetime only** (new token each start, atomic
  supersede). This is the intended behavior; timer rotation is not planned. The
  client's `401`-re-read-and-retry-once still applies. See Rotate / UX.
