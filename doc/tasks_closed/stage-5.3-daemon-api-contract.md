...
OpenAPI artifacts are generated by `./agent_collab.sh setup`, and token rotation
...
**Implemented as setup output.** `./agent_collab.sh setup` validates the
...
`./agent_collab.sh setup --check` verifies those artifacts without writing, so
...
- Generate `doc/daemon_api_doc/http-api.md` and
  ...
  `./agent_collab.sh setup`; `setup --check` provides the regen-and-diff gate.
...
- **Done:** generated `doc/daemon_api_doc/http-api.md` and `openapi.json` via
  `./agent_collab.sh setup`, with a non-writing `setup --check` drift gate.
...
- **A scope:** typed DTOs + contract tests are the runtime source of truth.
  `./agent_collab.sh setup` generates `doc/daemon_api_doc/http-api.md` and
  `openapi.json`; `setup --check` verifies drift without writing.
