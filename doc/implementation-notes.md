...
widen eligibility. Production `claude_sdk.interrupt`,
`antigravity_sdk.interrupt`, and `codex_sdk.interrupt` are true after
both-path credentialed continue-after-interrupt with a distinguishable
abort (`interrupted` / `local_turn_interrupted`); other `*.interrupt`
flags stay false.
...
with `interrupted` / `local_turn_interrupted`. Production
