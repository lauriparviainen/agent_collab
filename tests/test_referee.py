...
from agent_collab.sandbox.specs import SandboxEnforcement, SandboxFailure
    ...
    def test_direct_read_only_plan_runs_engine_preflight_before_runners(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            session_plan = SimpleNamespace(
                ...
                cleanup_failed_start_roots=mock.Mock(),
            )
            ...
            session_plan.cleanup_failed_start_roots.assert_called_once_with()
            session_plan.cleanup_created_session_private_roots.assert_not_called()

    def test_never_live_direct_runs_roll_back_host_roots(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            config = CollaborationConfig(
                agents={
            ...
            class DoneRunner(AgentRunner):
                async def run_turn(self, prompt, workdir, emit):
                    await emit(Event.create("claude", "message", "done"))
                    return TurnOutcome("completed")

            for never_live in ({"dry_run": True}, {"mock": True}):
                session_plan = SimpleNamespace(
                    agents={"claude": SimpleNamespace(enforcement=SandboxEnforcement.DISABLED)},
                    cleanup_created_session_private_roots=mock.Mock(),
                    cleanup_failed_start_roots=mock.Mock(),
                )
                referee = Referee(
                ...
                referee._runners = mock.Mock(return_value={"claude": DoneRunner()})
                asyncio.run(referee.run("task"))
                session_plan.cleanup_failed_start_roots.assert_called_once_with()
                session_plan.cleanup_created_session_private_roots.assert_not_called()

    def test_preflight_failure_rolls_back_host_roots(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            config = CollaborationConfig(
                agents={
            ...
            agent_plan = SimpleNamespace(enforcement=SandboxEnforcement.OS_ENFORCED)
            session_plan = SimpleNamespace(
                agents={"claude": agent_plan},
                cleanup_created_session_private_roots=mock.Mock(),
                cleanup_failed_start_roots=mock.Mock(),
            ...
            supervisor = mock.Mock()
            supervisor.preflight = mock.AsyncMock(
                side_effect=SandboxFailure("outer_sandbox_engine_unavailable", "no bwrap")
            )
            ...
            with (
                ...
                referee._runners = mock.Mock(side_effect=AssertionError("runners after preflight"))
            ...
            session_plan.cleanup_failed_start_roots.assert_called_once_with()
            session_plan.cleanup_created_session_private_roots.assert_not_called()
            referee._runners.assert_not_called()

    def test_live_end_keeps_host_roots(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            config = CollaborationConfig(
                agents={
            ...
            session_plan.cleanup_failed_start_roots.assert_not_called()
